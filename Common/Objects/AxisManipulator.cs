    ...
    public class AxisManipulator : SceneObject, IRayCasting
    {
        ...
        public void OnMouseMove(MouseMoveEventArgs e, bool isPressed)
        {
            if (isPressed)
            {
                ...
                if (axis.Equals("x"))
                    {
                        TranslateGlobal(Pos+new vec3(e.DeltaX/20f, 0, 0));
                    }else if (axis.Equals("y"))
                    {
                        TranslateGlobal(Pos+new vec3(0, -e.DeltaY/50f, 0));
                    }
                    else if (axis.Equals("z"))
                    {
                        TranslateGlobal(Pos+new vec3(0, 0, -e.DeltaX/50f));
                    }
            }
            else
            {
        ...
        public void CheckCollision(vec3 ray, vec3 cameraPosition)
        {
            ...
            foreach (var collider in _colliders)
            {
                ...
                {
                    //toDraw.Add(new Sphere(result*ray+cameraPosition, 0.01f));
                    //collider.Parent.EnableHighlighting();
